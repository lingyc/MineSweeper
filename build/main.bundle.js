...
/***/ function(module, exports, __webpack_require__) {
	...
	var _mineSweeper = __webpack_require__(3);
	...
	function initializedField(gridSize, bombCount) {
	  ...
	  var game = new _mineSweeper2.default(gridSize, bombCount);
	  var gridDiv = (0, _jquery2.default)('<div/>', { class: 'grid' });
	  var rows = [];
	
	  game.field.forEach(function (row, rowIdx) {
	    var rowDiv = (0, _jquery2.default)('<div/>', { class: 'row' });
	    row.forEach(function (cell, colIdx) {
	      var cellElement = (0, _jquery2.default)('<button/>');
	  ...
	  (0, _jquery2.default)('body').on('click', ".covered", handleClick);
	};
	
	function renderCells(cellIndexes, callback) {
	  cellIndexes.forEach(function (cellIndex) {
	    var _cellIndex2 = _slicedToArray(cellIndex, 2),
	        rowIdx = _cellIndex2[0],
	        colIdx = _cellIndex2[1];
	
	    var cellElement = (0, _jquery2.default)('.grid').children().eq(rowIdx).children().eq(colIdx);
	...
	function renderFeedBack(text) {
	  (0, _jquery2.default)('.feedback').text(text);
	}
	
	(0, _jquery2.default)(document).ready(function () {
	  renderUI();
	
	  (0, _jquery2.default)('.startGame').on('click', function () {
	    var gridSize = (0, _jquery2.default)('.gridSize').val();
	    var bombCount = (0, _jquery2.default)('.bombCount').val();
	    if (/^[0-9]*$/.test(gridSize + bombCount) && gridSize !== '' && bombCount !== '') {
	      if (bombCount >= gridSize * gridSize) {
	        ...
	        (0, _jquery2.default)('.startGame').text('restart game');
	      }
	    } else {
...
/***/ function(module, exports) {
	...
	var MineField = function () {
	  function MineField(fieldSize, bombCount) {
	  ...
	  _createClass(MineField, [{
	    key: "_createField",
	    value: function _createField(fieldSize, bombCount) {
	      var matrix = Array.from({ length: fieldSize }, function () {
	        return Array.from({ length: fieldSize }, function () {
	    ...
	    key: "_plantBombRandomly",
	    value: function _plantBombRandomly(matrix, fieldSize, bombCount) {
	      var positions = Array.from({ length: fieldSize * fieldSize }, function (val, key) {
	        return key;
	      });
	      var totalCellCount = fieldSize * fieldSize;
	      var currentBombCount = 0;
	
	      while (currentBombCount < bombCount) {
	        var bombIndex = convertTo2DIndex(Math.floor(Math.random() * totalCellCount));
	
	        var _bombIndex = _slicedToArray(bombIndex, 2),
	            row = _bombIndex[0],
	            col = _bombIndex[1];
	      ...
	      function convertTo2DIndex(linearIndex) {
	        var row = Math.floor(linearIndex / fieldSize);
	        var column = linearIndex % fieldSize;
	        return [row, column];
	      };
	    }
	  }, {
	    key: "_setBombAt",
	    value: function _setBombAt(index, matrix) {
	      var _index = _slicedToArray(index, 2),
	          row = _index[0],
	          column = _index[1];
	
	      matrix[row][column] = new BombCell();
	      this._getAdjacentIndexs(index, matrix).forEach(function (adjacentIndex) {
	        var _adjacentIndex = _slicedToArray(adjacentIndex, 2),
	            adjacentRow = _adjacentIndex[0],
	            adjacentColumn = _adjacentIndex[1];
	
	        var adjacentCell = matrix[adjacentRow][adjacentColumn];
	    ...
	    key: "_getAdjacentIndexs",
	    value: function _getAdjacentIndexs(index, matrix, option) {
	      var _index2 = _slicedToArray(index, 2),
	          row = _index2[0],
	          column = _index2[1];
	
	      var cells = [];
	      var increament = void 0;
	      var start = void 0;
	      ...
	      for (var i = start; i < 9; i += increament) {
	        if (i === 4) {
	          continue;
	        }
	        var adjacentRow = row - (1 - Math.floor(i / 3));
	        var adjacentcolumn = column - (1 - i % 3);
	        if (adjacentRow >= 0 && adjacentRow < matrix.length && adjacentcolumn >= 0 && adjacentcolumn < matrix.length) {
	    ...
	    key: "_getEmptyCellBody",
	    value: function _getEmptyCellBody(index, matrix) {
	      var _index3 = _slicedToArray(index, 2),
	          row = _index3[0],
	          column = _index3[1];
	
	      var newlyUnCoverCells = [];
	      var currentCell = matrix[row][column];
	      if (!currentCell.isBomb) {
	        currentCell.unCover();
	        newlyUnCoverCells.push(index);
	        if (currentCell.adjacentBombCount === 0) {
	          var cardinalIndexes = this._getAdjacentIndexs(index, matrix, { cardinal: true });
	          var _iteratorNormalCompletion = true;
	          var _didIteratorError = false;
	          var _iteratorError = undefined;
	
	          try {
	            for (var _iterator = cardinalIndexes[Symbol.iterator](), _step; !(_iteratorNormalCompletion = (_step = _iterator.next()).done); _iteratorNormalCompletion = true) {
	              var cardinalIndex = _step.value;
	
	              var _cardinalIndex = _slicedToArray(cardinalIndex, 2),
	                  cardinalRow = _cardinalIndex[0],
	                  cardinalColumn = _cardinalIndex[1];
	
	              var cardinalCell = matrix[cardinalRow][cardinalColumn];
	    ...
	    key: "uncoverAt",
	    value: function uncoverAt(index) {
	      var _index4 = _slicedToArray(index, 2),
	          row = _index4[0],
	          column = _index4[1];
	      ...
	      if (this.field[row][column].isBomb) {
	        ...
	        var newlyUnCoverCells = this._getEmptyCellBody([row, column], this.field);
	...
	var EmptyCell = function (_Cell) {
	  ...
	  function EmptyCell() {
	    var adjacentBombCount = arguments.length > 0 && arguments[0] !== undefined ? arguments[0] : 0;
	
	    _classCallCheck(this, EmptyCell);
	
	    var _this = _possibleConstructorReturn(this, (EmptyCell.__proto__ || Object.getPrototypeOf(EmptyCell)).call(this, { isBomb: false }));
	
	    _this.adjacentBombCount = adjacentBombCount;
	    return _this;
	  }
	
	  _createClass(EmptyCell, [{
	    key: "increaseAdjacentBombCount",
	    value: function increaseAdjacentBombCount() {
	...
	var BombCell = function (_Cell2) {
	  _inherits(BombCell, _Cell2);
	  ...
	  function BombCell() {
	    ...
	    return _possibleConstructorReturn(this, (BombCell.__proto__ || Object.getPrototypeOf(BombCell)).call(this, { isBomb: true }));
	  }
